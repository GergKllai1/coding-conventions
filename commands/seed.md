...
2. **Pick a canonical exemplar + anchor for each** — `path/File.ext#symbol @<short-sha>` (current `git rev-parse --short HEAD`), per the skill's entry format. Point, don't paste.
...
4. **Dedup before presenting** — load the existing catalog and classify each candidate New / Merge / Conflict, exactly as the skill's capture flow does. Never propose a duplicate of something already captured.
